...
function decodeLive(data?: Buffer) {
  ...
  const mask = data[313];
  return {revision: data.readBigUInt64LE(146), lockMask: mask, locks: [0, 1, 2].filter(index => mask & (1 << index)).length, phase: data[314]};
}
...
export default async function Home({searchParams}: {searchParams: Promise<{mode?: string}>}) {
  ...
  return <main>
    <nav><a className="brand" href="/">TRADE<span>TABLE</span></a><div className="network"><i className={liveResult ? "live" : ""} /> SOLANA DEVNET · MAGICBLOCK ER</div><a href="/proof">Proof ledger ↗</a></nav>
    <header className="hero">
      <div className="heroCopy">
        ...
        <div className="heroActions">
          {room ? <a className="primary" href="#live-room">Enter the live table ↓</a> : <span className="mutedButton">Demo deployment pending</span>}
          {room ? <a className="secondary" href={explorerAddress(room)}>Inspect room proof ↗</a> : null}
      ...
      <div className="equation" aria-label="Six assets in, three trade, three return">
        ...
        <small>SELECTED THREE SETTLE ATOMICALLY. UNSELECTED THREE RETURN SEPARATELY.</small>
      </div>
    </header>

    <section className="tableSection">
      <div className="sectionHead"><div><p className="kicker">THE SHARED SURFACE</p><h2>Six assets enter. Consensus chooses three.</h2></div><div className="authority"><span>READ AUTHORITY</span><strong className={liveResult ? "" : "empty"}>{liveResult?.source ?? "UNAVAILABLE"}</strong></div></div>
      <div className="mobileConsensus"><span>{liveResult?.source ?? "AUTHORITY UNAVAILABLE"}</span><strong>REV {liveState?.revision.toString() ?? "—"} · {liveState?.locks ?? 0}/3 LOCKS</strong></div>
      <div className="tradeTable">
        {(seats.length ? seats : [0, 1, 2]).map((seat, seatIndex) => {
          const participant = seat instanceof PublicKey ? seat : null;
          const locked = Boolean(liveState && liveState.lockMask & (1 << seatIndex));
          return <article className="seat" key={participant?.toBase58() ?? seatIndex}>
            <header><span className="seatNumber">0{seatIndex + 1}</span><div><strong>{participant ? short(participant) : "AWAITING ROOM"}</strong><small>PRIMARY COLLECTOR</small></div><span className={`lockState ${locked ? "locked" : ""}`}>{locked ? "LOCKED" : "OPEN"}</span><span aria-hidden="true" className={`lockDot ${locked ? "locked" : ""}`} /></header>
            <div className="assetPair">{[0, 1].map(localIndex => {
              ...
              return mint ? <a aria-label={`Slot ${slot}, ${selected ? "selected" : "not selected"}, mint ${mint.toBase58()}`} className={`asset ${selected ? "selected" : ""}`} href={explorerAddress(mint)} key={mint.toBase58()}>
                <span className={`art art${slot}`}><i>#{slot}</i></span><span className="assetMeta"><strong>SLOT {slot}{selected ? " · SELECTED" : ""}</strong><small>{short(mint)} · {mintInfos[slot] ? "RPC VERIFIED" : "UNAVAILABLE"}</small></span>
        ...
        <div className={`tableCenter ${liveState ? "" : "unavailable"}`}><span>EXACT REVISION</span><strong>{liveState?.revision.toString() ?? "—"}</strong><small>{liveState?.locks ?? 0} / 3 LOCKS</small></div>
      </div>
    </section>

    <section className="telemetry">
      <article><span>CORE AUTHORITY</span><strong className={coreResult ? "" : "missing"}>{coreResult ? "BASE" : "—"}</strong><small>{coreResult ? `${coreResult.value.data.length} bytes · confirmed` : "RPC unavailable"}</small></article>
      <article><span>LIVE REVISION</span><strong className={liveState ? "" : "missing"}>{liveState?.revision.toString() ?? "—"}</strong><small>{liveResult?.source ?? "authority unavailable"}</small></article>
      <article><span>LOCK CONSENSUS</span><strong className={liveState ? "" : "missing"}>{liveState?.locks ?? 0}<i>/3</i></strong><small>{liveState ? (liveState.phase === 1 ? "FROZEN" : liveState.phase === 2 ? "FINALIZED" : "NEGOTIATING") : "UNAVAILABLE"}</small></article>
      <article><span>CUSTODY MASK</span><strong className={core ? "" : "missing"}>{core?.deposited ?? 0}<i>/63</i></strong><small>{core ? `selected ${core.selected} · returned ${core.returned}` : "unavailable"}</small></article>
    </section>

    <div id="live-room">{room ? <RoomClient room={room.toBase58()} participants={seats.map(value => value.toBase58())} mints={mints.map(value => value.toBase58())} /> : <section className="notice"><strong>Read-only shell.</strong> Set `NEXT_PUBLIC_DEMO_ROOM` to an earned room; this page will never invent one.</section>}</div>
    ...
    <section className="operations">
      ...
      <div className="modes">
        <a aria-current={mode === "action"} href="/proof#local-only"><b>01</b><span><strong>COMPOSED ACTION</strong><small>LOCAL-VALIDATOR PROOF ONLY · OPEN EVIDENCE BOUNDARY</small></span></a>
        <a aria-current={mode === "committed"} href="/proof#base-consequences"><b>02</b><span><strong>NORMAL SETTLEMENT</strong><small>PUBLIC DEVNET: COMMIT-ONLY + BASE SETTLEMENT</small></span></a>
        <a aria-current={mode === "recover"} href="/proof#base-consequences"><b>03</b><span><strong>BASE RETURN PROOF</strong><small>OPEN THREE SEPARATE RETURN TRANSACTIONS</small></span></a>
      </div>
    </section>
